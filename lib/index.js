...
/** 调用方的身份：模型工具给 agent/session id；面板给 `panel`（浏览器侧无法区分到人）。 */
  ...
  async function sendKeys(args, meta = {}) {
    ...
    // 0.3.1「注入即焚」：护栏已通过、字节未进管道 —— 就在这一刻落刀（见 spendVaultOnSendAttempt）
    if (exp.usedVault.length > 0) spendVaultOnSendAttempt(name, exp, actor)
    const entry = owners[name]
    if (entry !== undefined) { entry.lastUsedAt = Date.now(); entry.lastActor = actor === '' ? entry.lastActor : actor }
    ...
    ctx.inject(['webServer'], (webCtx) => {
      ...
      route('/vault-set', async (req, res) => {
        try {
          ...
          recordAudit({ event: 'vault', shell: '', source: 'panel', actor: 'panel', decision: 'set', key, result: `${value.length}chars/${body.oneShot === true ? 'one-shot' : 'reusable'}` })
  ...
  async function runSteps(id, steps, exec) {
    ...
    for (let i = 0; i < steps.length; i += 1) {
      ...
      if (send !== '') {
        ...
        // 0.3.1「注入即焚」：进管道前落刀（护栏已通过）
        if (usedVaultNow.length > 0) spendVaultOnSendAttempt(id, { usedVault: usedVaultNow, oneShotKeys: oneShotNow }, actor)
        const oe = owners[id]
  ...
  async function readDomainFile(file) {
    try {
      const parsed = JSON.parse(await readTextFile(file, 'utf8'))
      return (parsed !== null && typeof parsed === 'object' && !Array.isArray(parsed)) ? parsed : {}
    } catch { return {} }
  }
  async function writeDomainFile(file, obj) {
    await ensureAuditDir(auditPaths.dir)
    await writeTextFile(file, JSON.stringify(obj, null, 2) + '\n', { mode: 0o600 })
  }
  const readVault = () => readDomainFile(VAULT_FILE())
  const readMacros = () => readDomainFile(MACRO_FILE())
  const writeVault = (obj) => writeDomainFile(VAULT_FILE(), obj)
  const writeMacros = (obj) => writeDomainFile(MACRO_FILE(), obj)
  ...
  ctx.tools.register(guardedTool({
    ...
    async execute(args, exec) {
      ...
      const waitSpec = typeof args.waitFor === 'string' && args.waitFor !== '' ? parseWaitFor(args.waitFor) : null
      ...
      for (const id of ids) {
        ...
        recordAudit({ event: 'input', shell: id, source: 'tool', actor, text: auditTextFor(id, text), keys,
          guard: args.confirm === true ? 'confirm-bypass' : 'allowed', result: 'sent' })
        // 0.3.1「注入即焚」：护栏已通过、sendText 进管道之前落刀（不赌 driver.send 的返回值）
        if (exp.usedVault.length > 0) spendVaultOnSendAttempt(id, exp, actor)
    ...
    async execute(args, exec) {
      ...
      if (action === 'macro-list' || action === 'vault-list') {
        ...
        if (action === 'vault-list') {
          ...
          return items.map(([k, e]) => `{{v:${k}}} kind=${e?.kind || 'secret'} oneShot=${e?.oneShot === true} uses=${Number(e?.uses ?? 0)}${e?.note ? ` — ${String(e.note).slice(0, 60)}` : ''}`).join('\n')
        }
        const macros = await readMacros()
