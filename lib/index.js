...
import { NAME_PREFIX, DANGEROUS, sanitizeName, clamp, scanDanger, isSafeSessionName, parseTmuxVersion, idleClosable, shouldWrapExit, wrapExitCommand, parseExitFromTail, diffSince, stripCommandEcho, redactSecrets, parseWaitFor, summarizeFailure, doctorAdvice } from './pure.mjs'
...
export const Config = z.object({
  ...
  idleClose: z.boolean().default(true)
    .description('闲置自动关闭：会话在 idleCloseMinutes 分钟内没有任何输入或运行（AI 工具与面板的输入都算，被护栏拦下的企图也算；只读屏/查询不算活动，否则面板开着就永不关）就自动关闭（与手动 close 同一路径、记审计）。防止把上限堆满、留僵尸 bash。临时不想关就把这个关掉。立即生效。'),
  idleCloseMinutes: z.number().default(60)
    .description('闲置自动关闭的时长（分钟，**统一时间单位**）。每个会话在创建时确定自己的时长（shell_open 的 idleMinutes 参数，不传就用这个默认值），之后可用 shell_manage action=idle 修改；到点仍无输入/运行即自动关闭。立即生效。'),
...
function resolveConfig(cfg) {
  ...
  return {
    ...
    idleClose: source.idleClose !== false,
    ...
    try {
      ...
      all[name] = {
        ...
        idleMinutes: Number.isFinite(row?.idleMinutes) ? row.idleMinutes
          : (entry?.idleMinutes ?? (Number.isFinite(prev?.idleMinutes) ? prev.idleMinutes : resolved.idleCloseMinutes)),
        at: Date.now(),
...
/** 调用方的身份：模型工具给 agent/session id；面板给 `panel`（浏览器侧无法区分到人）。 */
  ...
  async function idleSweepOnce() {
    if (resolved.idleClose !== true) return
  ...
  async function openShell(args = {}, meta = {}) {
    ...
    // 闲置自动关闭时长：创建时定死（显式 idleMinutes > 快照 > 设置默认），之后可 shell_manage idle 改。
  ...
  // 闲置自动关闭时长（分钟，统一单位）：1–1440（24h），非法输入退回默认；
  // 0 表示该会话**永不**自动关闭（idleClosable 对 <=0 一律跳过）。
  ...
  /** 工具返回给 AI/面板的屏文本统一出口：按设置做敏感信息脱敏（G）。 */
  ...
  ctx.tools.register(guardedTool({
    ...
    parameters: {
      ...
      idleMinutes: { type: 'number', description: 'Idle auto-close timeout in MINUTES (all tools use minutes). 该会话连续这么多分钟没有输入或运行就自动关闭（工具/面板输入含被拦企图、shell_run 发送都算；只读屏/查询不算活动）。创建时必须确定这个值：不传则用设置 idleCloseMinutes（默认 60）；之后用 shell_manage action=idle 改。0 = 该会话不自动关闭。' },
    ...
    async execute(args, exec) {
      ...
      return `session ${opened.name}${opened.label !== '' ? ` (${opened.label})` : ''} | ${opened.cols}x${opened.rows} | cwd ${opened.cwd} (${opened.cwdSource}) | idle ${opened.idleMinutes}m${opened.from === '' ? '' : ` | 复原自快照 ${opened.from}`}\n${opened.screen}`
    },
  }))
  ...
  ctx.tools.register(guardedTool({
    ...
    async execute(args, exec) {
      ...
      for (const id of ids) {
        ...
        const wrapPlan = (args.structured === false || waitSpec !== null)
          ? { wrap: false, reason: waitSpec !== null ? 'waitFor' : 'disabled' }
          : shouldWrapExit(text)
        const sendText = wrapPlan.wrap ? wrapExitCommand(text) : text
        ...
        if (waitSpec !== null) {
          ...
          while (Date.now() - waitStart < waitMs) {
            if (waitSpec.kind === 'match') {
              ...
              const fresh = stripCommandEcho(sincePreview(id, full), text)
  ...
  ctx.tools.register(guardedTool({
    name: 'shell_read',
    description: 'Read a shell without sending anything. mode "tail" (default) returns the bottom `lines` lines; "screen" returns the visible screen; "history" returns scrollback above the visible screen; "since" returns only the output that changed since the last read/send/run on this session. '
    ...
    parameters: {
      session: { type: 'string', required: true, description: 'Stable id(s): "abc" / "a,b,c" / "mine" / "*".' },
      mode: { type: 'string', enum: ['tail', 'screen', 'history', 'since'], description: 'tail (default) / screen / history / since. 传了 until 就是等待模式、传了 search 就是搜索模式，二者都会忽略 mode。' },
      search: { type: 'string', description: '搜索模式（F 流）：在**全量滚动缓冲**（最多 historyLimit 行）里按正则找匹配行，返回行号 + 命中行——比 history+tail 硬翻高效。非法正则立即 REFUSED。' },
    ...
    async execute(args, exec) {
      ...
      if (search !== null) {
        ...
        for (const id of ids) {
          ...
          const hits = []
          for (let i = 0; i < bufLines.length && hits.length < cap; i += 1) {
            if (re.test(bufLines[i])) hits.push(`L${i + 1}  ${bufLines[i].slice(0, 400)}`)
          }
          found.push(`[${id}] 搜索 "${search}" → ${hits.length >= cap ? `≥${cap}` : hits.length} 处${hits.length > 0 ? '\n' + showText(hits.join('\n')) : ''}`)
        }
        return found.join('\n\n')
      }
  ...
  ctx.tools.register(guardedTool({
    name: 'shell_manage',
    description: 'Manage shells: action "rename" (change the display label — the stable id never changes), "resize" (new cols/rows), "close" (kill the session and everything inside it), "reap" (close idle shells of this conversation), "idle" (change this session\'s idle auto-close timeout, in MINUTES — 0 means never auto-close), "snapshot" (save this session\'s scenario snapshot so it can be resurrected with shell_open from=<id>), "doctor" (self-check: foreground/idle time/buffer usage/capture stopped → actionable advice). session accepts id(s)/"mine"/"*"; for reap, scope defaults to "mine". Only manage shells this conversation created unless the user explicitly allows other owners.',
    parameters: {
      action: { type: 'string', required: true, enum: ['rename', 'resize', 'close', 'reap', 'idle', 'snapshot', 'doctor'], description: 'What to do.' },
      session: { type: 'string', description: 'Stable id(s) for rename/resize/close/idle.' },
      ...
      minutes: { type: 'number', description: 'idle: new idle auto-close timeout in MINUTES (all tools use minutes; default = settings idleCloseMinutes). 0 = never auto-close this session.' },
    },
    output: text,
    async execute(args, exec) {
      ...
      if (action === 'idle') {
        ...
        if (entry !== undefined) entry.idleMinutes = minutes
        void writeSnapshot({ name: id, idleMinutes: minutes, cwd: await liveCwdOf(id), reason: 'idle' }, entry)
        recordAudit({ event: 'idle', shell: id, source: 'tool', actor: actorOf(exec), minutes, result: 'set' })
        void writeOwners(auditPaths, owners)
        return `[${id}] idle auto-close = ${minutes} 分钟${minutes === 0 ? '（永不自动关闭）' : '（到点无操作即自动关闭）'}`
      }
      if (action === 'doctor') {
        ...
        for (const id of idList) {
          ...
          const head = `[${id}] fg=${row.foreground || '?'} · 无活动 ${idleSec}s · 缓冲 ${Math.round(bufferPct)}%` +
            ` · idle=${entry?.idleMinutes ?? resolved.idleCloseMinutes}m · 留痕${captureStopped.has(id) ? '已停' : '正常'}`
  ...
  ctx.tools.register(guardedTool({
    ...
    async execute(_args, exec) {
      ...
      const lines = rows.map((s) => {
        ...
        // 每会话闲置时长（分钟，统一单位）：0 = 永不自动关闭
        const idleMinutes = entry?.idleMinutes ?? resolved.idleCloseMinutes
        const idleTag = idleMinutes === 0 ? 'idle=0(off)' : `idle=${idleMinutes}m${resolved.idleClose ? '' : '(总开关关)'}`
        return `id=${s.name}  label=${s.label || s.name}  ${s.cols}x${s.rows}  fg=${s.foreground || '?'}  attached=${s.attached ? 'yes' : 'no'}  buffer=${s.historySize}/${s.historyLimit}  owner=${tag}  ${idleTag}${lastRunText(s.name)}${busyTag}`
      })
      ...
      const diag = [
        ...
        `settings: ${resolved.shell}/${resolved.cols}x${resolved.rows} · max ${resolved.maxSessions} · idle ${resolved.idleClose ? 'on' : 'off'}(${resolved.idleCloseMinutes}m) · guard ${resolved.guard ? 'on' : 'off'} · tmux ${state.tmux?.ok === true ? state.tmux.version : 'unavailable'}`,
