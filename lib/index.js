...
export function apply(ctx, config) {
  ...
  ;(async function start() {
    try {
      ...
      if (resolved.watchdog) {
        ...
        state.lastCleanup = result.cleaned
        state.watchedPid = result.harnessPid
        state.watchdogPid = result.watchdogPid
        state.adopted = result.adopted
        if (result.adopted) {
          console.log(`dsh-agent-shell: adopted the existing watchdog for harness pid ${result.harnessPid}; sessions kept`)
        } else if (result.cleaned.length > 0) {
          console.log(`dsh-agent-shell: cleaned ${result.cleaned.length} orphan session(s): ${result.cleaned.join(', ')}`)
        }
      }
      state.ready = true
    } catch (error) {
      console.error(`dsh-agent-shell: startup cleanup failed: ${String(error)}`)
      state.ready = true
    }
  })()
  ...
  async function openShell(args = {}) {
    ...
    const label = typeof args.name === 'string' && args.name.length > 0
      ? args.name.replace(/[^A-Za-z0-9_.-]/g, '-')
      : Math.random().toString(36).slice(2, 8)
    const base = NAME_PREFIX + label
    let name = base
    let n = 2
    ...
    const cols = Number.isFinite(args.cols) ? Math.max(20, Math.floor(args.cols)) : resolved.cols
    const rows = Number.isFinite(args.rows) ? Math.max(5, Math.floor(args.rows)) : resolved.rows
  ...
  async function sendKeys(args) {
    const name = String(args.name ?? args.session ?? '')
    ...
    ctx.inject(['webServer'], (webCtx) => {
        ...
        try {
          ...
          json(res, 200, {
            ...
            server: {
              ...
              cleanedAtBoot: state.lastCleanup,
            },
          })
        } catch (error) {
      ...
      route('/resize', async (req, res) => {
        try {
          const body = await readBody(req)
          const name = String(body.name ?? '')
          const cols = Math.max(20, Math.floor(Number(body.cols)))
          const rows = Math.max(5, Math.floor(Number(body.rows)))
