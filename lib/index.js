...
import { NAME_PREFIX, DANGEROUS, sanitizeName, clamp, scanDanger, isSafeSessionName, parseTmuxVersion } from './pure.mjs'
...
/** 调用方的身份：模型工具给 agent/session id；面板给 `panel`（浏览器侧无法区分到人）。 */
  ...
  state.envReady = (async () => {
    try {
      state.env = await detectEnv({ tmux: null })
    } catch (error) {
  ...
  ctx.tools.register(guardedTool({
    name: 'shell_wait',
    description: 'Wait until a session reaches a state, instead of sleeping blindly. until 三选一：'
      + '"idle"（突出提示符）、"fg:<命令名>"（前台变成某命令，如 fg:vim）、"match:<正则>"'
      + '（屏幕**新出现**的输出匹配该正则 —— 等待开始前屏上已有的旧文本不算命中）。'
      + '有超时兜底：默认 30s，可传 timeout（毫秒）覆盖，**不会无限等待**；until 或 timeout 参数'
      + '不合法会立即 REFUSED 并说明，不会干等。返回是否到达 + 尾部输出。'
      + '适合等待长任务结束 / 交互程序进入预期状态（sudo/ssh/vim）后再读屏。',
    parameters: {
    ...
    async execute(args, exec) {
      ...
      let re = until.startsWith('match:') ? until.slice(6) : null
      const fgWant = until.startsWith('fg:') ? until.slice(3) : null
      if (until !== 'idle' && fgWant === null && re === null) {
        return `[${id}] REFUSED — 无法识别 until="${until}"。`
          + '只支持三种：idle（等提示符）、fg:<命令名>（等着前台变成某个命令，如 fg:vim）、'
          + 'match:<正则>（等屏幕出现新输出匹配；只匹配等待开始后新出现的内容）'
      }
      if (re !== null) {
        try { re = new RegExp(re) } catch (error) {
          return `[${id}] REFUSED — match 正则不合法（${String(error && error.message ? error.message : error)}）`
        }
      }
      const timeoutMs = Number.isFinite(args.timeout) ? Math.floor(args.timeout) : 30000
      if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) return `[${id}] REFUSED — timeout 必须是大于 0 的毫秒数（默认 30000）`
      const started = Date.now()
      let reached = false
      let fg = ''
      let tail = ''
      // match 只对**等待开始后新出现的输出**做匹配：屏上本来就残留的历史文本（比如上一次
      // 命令的提示行）不该让 match 立刻"假成功"——这正是把 match 从整屏 tail 改成增量
      // 的原因（0.2.1 强化）。与 shell_read "since" 共用同一份预览快照表。
      while (Date.now() - started < timeoutMs) {
        fg = await driver.foregroundOf(id)
        ...
        if (re !== null) {
          ...
          rememberPreview(id, full)
          if (re.test(fresh)) { reached = true; break }
        }
  ...
  ctx.tools.register(guardedTool({
    ...
    async execute(_args, exec) {
      ...
      const lines = rows.map((s) => {
        ...
        return `id=${s.name}  label=${s.label || s.name}  ${s.cols}x${s.rows}  fg=${s.foreground || '?'}  attached=${s.attached ? 'yes' : 'no'}  buffer=${s.historySize}/${s.historyLimit}  owner=${tag}${busyTag}`
      })
