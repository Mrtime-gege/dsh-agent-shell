...
import { NAME_PREFIX, DANGEROUS, sanitizeName, clamp, scanDanger, isSafeSessionName } from './pure.mjs'
...
/** 调用方的身份：模型工具给 agent/session id；面板给 `panel`（浏览器侧无法区分到人）。 */
  ...
  state.envReady = (async () => {
    try {
      ...
      // 在**任何会话创建之前**把决议灌进 driver（state.ready 会 await 本 promise）
      driver.serverLaunch = state.plan.serverLaunch
      driver.foregroundStrategy = state.plan.foreground
      // 看门狗的双条件是"租约过期 ∧ 进程读不到"；读不到 /proc 时退化为"仅租约"（grace 放宽）
      ...
      if (state.plan.serverLaunch !== 'systemd-scope') {
        recordAudit({ event: 'env-degraded', capability: 'systemd-user', fallback: 'plain-detach',
          result: 'dsh 干净重启后会话不再存活' })
      }
      if (state.plan.foreground !== 'drill') {
        recordAudit({ event: 'env-degraded', capability: 'proc-readable', fallback: 'raw-foreground',
          result: '嵌套 tmux 忙闲判定可能不准' })
      }
      if (state.plan.harnessPid !== 'proc-chain') {
        recordAudit({ event: 'env-degraded', capability: 'proc-readable', fallback: 'self-only-harness-pid', result: '看门狗退化为租约单条件' })
      }
    } catch (error) {
      // 探测失败一律向"能力少"保守降级（探测不到 = 当作没有），但不能因此不工作
      state.env = null
      state.plan = planFor({ hasTmux: false, tmuxVer: null, hasSystemdUser: false, procReadable: false, tmpDir: '', isTermux: false })
