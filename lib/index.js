...
import {
  ...
  listOutputFiles, outputFileFor, outputSize, pruneAuditFiles, readAudit, readOwners,
  summarizeRecord, writeOwners,
} from './audit.js'
...
export function apply(ctx, config) {
  ...
  // 启动时清理过期审计 + 读回归属表（宿主重启后 owner 仍然可查）
  void (async () => {
    if (!resolved.audit) return
    try {
      await ensureAuditDir(auditPaths.dir)
      owners = await readOwners(auditPaths)
  ...
  const approvalSeamMounted = approvalService !== undefined
  ...
  async function openShell(args = {}, meta = {}) {
    ...
    recordAudit({ event: 'open', shell: name, source, actor, cols, rows, cwd, captureFile, result: 'ok' })
  ...
  if (resolved.exposeHttp) {
    ctx.inject(['webServer'], (webCtx) => {
      ...
      const fenceOptions = {
        connection: connection !== undefined && typeof connection.requestRejection === 'function' ? connection : undefined,
        requireLoopback,
      ...
      const route = (path, handler) => {
        const guarded = (req, res) => {
          const why = fenceReason(req, { ...fenceOptions, allowedHosts: resolved.allowedHosts })
          if (why !== null) {
      ...
      route('/keys', async (req, res) => {
        try {
          ...
          const result = await sendKeys(body, { actor: 'panel', source: 'panel' })
      ...
      route('/new', async (req, res) => {
        try {
          const body = await readBody(req)
          json(res, 200, await openShell(body, { actor: 'panel', source: 'panel' }))
        } catch (error) {
          json(res, errorCode(error), { error: String(error?.message ?? error) })
        }
      })
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      const opened = await openShell(args, { actor: actorOf(exec), source: 'tool' })
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      }, { actor: actorOf(exec), source: 'tool' })
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(_args, exec) {
      ...
      const lines = [
        ...
        `settings namespace: dsh-agent-shell ${settingsService === undefined ? '(service not mounted — using composition config)' : '— ' + state.settingsNote}`,
        state.fence === null
