...
export function apply(ctx, config) {
  ...
  const state = {
    ...
    lastRuns: new Map(),
    /** 0.3.0 vault 回显打码表：session → Map(值 → 键名)。值进过 pane 后，该会话的读屏出口持续替换。 */
    vaultMarks: new Map(),
...
/** 调用方的身份：模型工具给 agent/session id；面板给 `panel`（浏览器侧无法区分到人）。 */
  ...
  ;(async function start() {
    try {
      ...
      try {
        ...
        if (touched > 0) void writeOwners(auditPaths, owners)
  ...
  async function idleSweepOnce() {
    ...
    for (const name of due) {
      ...
      console.log(`dsh-agent-shell: idle sweep closed ${name} (no use for ${resolved.idleCloseMinutes}m)`)
    }
    if (due.length > 0) void writeOwners(auditPaths, owners)
  }
  ...
  async function openShell(args = {}, meta = {}) {
    ...
    return { name, id: name, label, cols, rows, cwd, cwdSource, idleMinutes, from: fromId, screen: await driver.screen(name) }
  }
  ...
  async function sendKeys(args, meta = {}) {
    ...
    const exp = await expandForSend(name, text, actor)
    ...
    // 0.3.0：oneShot 烧掉 + 值登记进该会话读屏打码表（发送成功才算"用掉"）
    if (exp.usedVault.length > 0) void consumeVaultAfterSend(name, exp.usedVault, actor)
    return { ok: true }
  }
  ...
  async function runSteps(id, steps, exec) {
    ...
    for (let i = 0; i < steps.length; i += 1) {
      ...
      if (send !== '') {
        ...
        if (usedVaultNow.length > 0) void consumeVaultAfterSend(id, usedVaultNow, actor)
  ...
  /* ── 0.3.0 双域系统（vault 秘密域 / macros 宏域）─────────────────────────────
   * 两个域物理隔离（两个文件、两套引用前缀 {{v:}}/{{m:}}）：共享键空间会被键碰撞攻击。
   *   vault.json  —— **人写**（面板 / CLI），AI 只见键名、永不见值；值不进审计文本。
   *                  oneShot 键在成功注入后被消耗（vaultConsumeNow 纯决策：同一条命令里
   *                  重复引用 = 违规计数不烧，防"故意两次引用绕过一次性"）。
  ...
  /** 登记打码标记：值进过 pane 后，该会话读屏出口持续替换成 [vault:{{v:键}}]（每会话上限 32 个）。 */
  function vaultMarkAdd(name, value, key) {
    const v = String(value ?? '')
    if (v.length < 3 || name === '' || key === '') return
    let marks = state.vaultMarks.get(name)
    if (marks === undefined) { marks = new Map(); state.vaultMarks.set(name, marks) }
    marks.set(v, key)
   ...
   * @returns {{text:string, usedVault:string[], noEnter?:boolean}|{error:string}}
   */
  async function expandForSend(name, text, actor) {
    const raw = String(text ?? '')
    if (raw === '' || extractRefs(raw).length === 0) return { text: raw, usedVault: [] }
    ...
    // ⚠ 打码表在**发送前**登记（不能等发送成功后的异步消耗）：值一进 pane，下一次读屏
    // 就可能把它带回来 —— 晚一帧都是漏。oneShot 的"烧"才放在发送成功后（见 consumeVaultAfterSend）。
    for (const key of r.usedVault) {
      const value = String(vault[key]?.value ?? '')
      if (value.length >= 3) vaultMarkAdd(name, value, key)
    }
    ...
    return { text: r.text, usedVault: r.usedVault, noEnter }
  }

  /** 发送成功后：oneShot 烧掉 + uses 计数（打码表已在 expandForSend 发送**前**登记，这里只管账）。 */
  async function consumeVaultAfterSend(name, usedKeys, actor) {
    if (!Array.isArray(usedKeys) || usedKeys.length === 0) return
    try {
      const vault = await readVault()
      let dirty = false
      for (const key of usedKeys) {
        ...
        const decision = vaultConsumeNow({ entry, repeat: usedKeys.filter((k) => k === key).length })
        if (decision.violation === true) {
          recordAudit({ event: 'vault', shell: name, source: 'tool', actor, decision: 'consume-violation', key, result: 'one-shot 同条命令重复引用 → 不烧并记账' })
        }
        if (decision.consume === true) {
          delete vault[key]
          dirty = true
          recordAudit({ event: 'vault', shell: name, source: 'tool', actor, decision: 'consume', key, result: 'one-shot used' })
  ...
  /** 工具返回给 AI/面板的屏文本统一出口：vault 值打码（会话级）+ 设置形状脱敏（G）。 */
  function showText(value, name = '') {
    let text = String(value ?? '')
    if (name !== '') {
      const marks = state.vaultMarks.get(name)
      if (marks !== undefined && marks.size > 0) {
        for (const [val, key] of [...marks.entries()].sort((a, b) => b[0].length - a[0].length)) {
          text = text.split(val).join(`[vault:{{v:${key}}}]`)
        }
      }
    }
  ...
  ctx.tools.register(guardedTool({
    ...
    async execute(args, exec) {
      ...
      for (const id of ids) {
        ...
        // 0.3.0：oneShot 烧掉 + 值登记进该会话的读屏打码表（发送成功才算用掉）
        if (exp.usedVault.length > 0) void consumeVaultAfterSend(id, exp.usedVault, actor)
    ...
    async execute(args, exec) {
      ...
      for (const id of ids) {
        ...
        if (mode === 'summary') {
          ...
          const vis = (await driver.captureWithMeta(id, undefined, { trim: true })).screen
    ...
    async execute(args, exec) {
      ...
      if (action === 'macro-list' || action === 'vault-list') {
        ...
        if (action === 'vault-list') {
          ...
          if (items.length === 0) return '(vault 为空：键由人在面板「秘密与宏」或 CLI `dsh-agent-shell vault set` 录入；AI 永远只见键名，值不上屏、不进账)'
