...
export class TmuxDriver {
  ...
  /** control 路径可用的前提：服务端已确认存在，且不在退避期内（失败后停用 60 秒再试）。 */
  controlUsable() {
  ...
  /** 记录一次 control 失败：停用 60 秒并收掉当前客户端（下一次再试时重新 spawn）。 */
  markControlDown() {
    this.controlRetryAfter = Date.now() + 60000
  ...
  async has(name) {
    if (this.controlUsable()) {
      try {
        await this.control.command(`has-session -t ${name}`, { timeoutMs: 2000 })
        return true
      } catch (error) {
      ...
      try {
        if (await this.serverExists()) {
          ...
          await this.tmux(['new-session', '-d', '-s', name,
            '-x', String(cols), '-y', String(rows), '-c', cwd, this.shell])
        } else {
    ...
    if (this.controlUsable()) {
      try {
        const out = await this.control.command(
          `display-message -p -t ${name} "#{pane_current_command}\t#{pane_pid}"`, { timeoutMs: 3000 })
  ...
  async resolveForeground(command, pid) {
    ...
    return drillForeground(current, panePid, await readDescendantProcs(panePid))
  }
  ...
  async captureWithMetaViaControl(name, lines, options = {}) {
    ...
    const metaOut = await this.control.command(
      `display-message -p -t ${name} ${tmuxQuote(LIST_FORMAT)}`, { timeoutMs: 4000 })
    const capture = lines === undefined
      ? `capture-pane -p -t ${name}`
      : `capture-pane -p -S -${lines} -t ${name}`
    const screenOut = await this.control.command(capture, { timeoutMs: 4000 })
    this.serverConfirmed = true
    const cut = metaOut.indexOf('\n')
    const head = cut < 0 ? metaOut : metaOut.slice(0, cut)
    const meta = head.includes(SEP) ? toSession(head) : null
    // meta 帧一般没有残留行；万一带了（防御），拼回屏幕不破坏行数语义
    const extra = cut < 0 ? '' : metaOut.slice(cut + 1)
