...
/**
 * 0.3.0 oneShot 消费判定（纯决策）：这次发送要不要把 vault 键**用后即焚**。
 * 规则：oneShot 键在同一条发送里出现 ≥2 次 = 调用方想连用两次 —— 不烧（发了两次才烧一次是错的），
 * 并记一次违规计数；一次性出现才烧。返回值不含秘密。
 * @param {{entry?: {oneShot?: boolean}, repeat: number}} args
 * @returns {{consume:boolean, violation:boolean, usesBump:boolean}}
 */
export function vaultConsumeNow ({ entry, repeat = 1 } = {}) {
  if (entry === undefined) return { consume: false, violation: false, usesBump: false }
  if (entry.oneShot !== true) return { consume: false, violation: false, usesBump: true }
  if (repeat > 1) return { consume: false, violation: true, usesBump: true }
  return { consume: true, violation: false, usesBump: false }
}
...
export function applyRefs (text, vault, macros, actor, sessionId) {
  ...
  if (refs.length === 0) return { ok: true, text: String(text ?? ''), usedVault: [], usedMacros: [] }
  let out = String(text)
  const usedVault = []
  const usedMacros = []
  ...
  return { ok: true, text: out, usedVault: [...new Set(usedVault)], usedMacros: [...new Set(usedMacros)] }
}
