...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ShellPanel() {
      ...
      const actorsTimer = react.useRef(null)
      ...
      const head = h('div', {
        ...
        h('button', {
          ...
          current === null
            ? h('span', { key: 't', className: 'dshsh-name-text' }, '(无 shell)')
        ...
        h('button', {
          ...
          onClick: () => { setLocked(true); closeAllMenus(); setCloseArmed(false); setExpanded(false) },
          title: '收起（会自动上锁）',
        }, h(Icon, { name: 'min' })),
      ])
      ...
      // 打开时拉一次活跃对话目录（'/actors'，宿主侧 5 秒缓存）；关闭时清掉选中项与草稿。
      react.useEffect(() => {
        if (!consentMenuOpen) return
        let alive = true
        setActors(null)
        void api('/actors').then((result) => {
          if (!alive) return
          if (result.ok && result.body !== null && typeof result.body === 'object') {
            setActors(result.body)
          } else {
            setActors({ supported: false, actors: [], note: '拉取对话列表失败' })
          }
        }).catch(() => { if (alive) setActors({ supported: false, actors: [], note: '拉取对话列表失败' }) })
        return () => { alive = false; setActorPick(null); setActorQuery(''); setActorScope('read'); setActorTtl('10m') }
      }, [consentMenuOpen])
      ...
      const consentMenu = consentMenuOpen ? h('div', { key: 'cmenu', className: 'dshsh-cmenu' }, [
        ...
        // 活跃对话目录：搜索 → 选中 → 档位 × 有效期 → 授予。未返回对话时给"手工填 id"入口。
        actors === null
          ? h('div', { key: 'loading', className: 'dshsh-cmenu-sub' }, '正在读取对话列表…')
          : actors.supported !== true
            ? h('div', { key: 'unsupported', className: 'dshsh-cmenu-sub' },
                (actors.note ?? '宿主未提供对话目录') + '。可以先在 `shell_state` / 审计里复制会话 id，再选「手工填 id」。')
            : h('div', { key: 'act', className: 'dshsh-actors' }, [
                ...
                // 选中某个对话后：档位 × 有效期 × 授予（与通配段共用 scope/ttl 控件，但作用于单个 actor）
                actorPick !== null ? h('div', { key: 'pick', className: 'dshsh-actor-pick' }, [
