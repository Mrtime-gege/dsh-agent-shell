...
.dshsh-kv>k{flex:0 0 104px;color:var(--dsw-alias-label-tertiary);font-family:system-ui,-apple-system,"Segoe UI","Microsoft YaHei",sans-serif;
...
.dshsh-badge{display:inline-flex;align-items:center;height:16px;padding:0 6px;border-radius:999px;
  border:1px solid var(--dsw-alias-state-error-primary);color:var(--dsw-alias-state-error-primary);font-size:10px;font-family:ui-monospace,Menlo,monospace}
  ...
  factory: (require) => {
    ...
    function ShellPanel() {
      ...
      const dotColor = (session) => {
        ...
        return foreground === '' || foreground === 'bash' || foreground === '-bash' ? 'var(--dsw-alias-state-success-primary)' : 'var(--dsw-alias-state-warn-primary)'
      }
      ...
      if (!expanded) {
        return h('button', {
          ...
          h('span', {
            key: 'l',
            style: { display: 'inline-flex', color: locked ? 'var(--dsw-alias-state-warn-primary)' : 'var(--dsw-alias-state-success-primary)' },
