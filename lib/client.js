...
function scrollAnchor(prevText, nextText, view) {
  ...
  const lineHeight = nextLines.length > 0 ? Math.max(0, scrollHeight - padY) / nextLines.length : 0
...
function caretPlacement(input) {
  ...
  return { lineIndex, cell: cursorX, charIndex: cellsToCharIndex(line, cursorX) }
}
...
/** 光标在内容坐标里的位置（`pre` 是滚动容器，绝对定位的子元素会随内容一起滚动）。 */
function caretStyle(placement, lineHeight, cellWidth, padX, padY) {
  if (placement === null || !Number.isFinite(lineHeight) || !Number.isFinite(cellWidth) || cellWidth <= 0) return null
  return {
    position: 'absolute',
    top: padY + placement.lineIndex * lineHeight,
    left: padX + placement.cell * cellWidth,
    width: 2,
    height: lineHeight,
...
/* 真实光标：绝对定位在 <pre> 内容坐标里，因此随内容一起滚动，不需要额外同步滚动位置 */
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ShellPanel() {
      ...
      /** 量字符宽度的隐藏样本（等宽字体下量 40 个 M 再除）；量不到就不画光标。 */
      ...
      const caretLineHeight = (() => {
        const el = screenRef.current
        if (el === null || typeof el.scrollHeight !== 'number') return 0
        const count = screen === '' ? 0 : screen.split('\n').length
        return count > 0 ? Math.max(0, el.scrollHeight - SCREEN_PAD_Y) / count : 0
      })()
      const caret = react.useMemo(() => {
        const placement = caretPlacement({ screen, meta })
        if (!shouldShowCaret({ locked, meta, placement })) return null
        const style = caretStyle(placement, caretLineHeight, cellWidthRef.current, SCREEN_PAD_X, SCREEN_PAD_Y / 2)
        return style === null ? null : { placement, style }
      }, [screen, meta, locked, caretLineHeight])
      ...
      const body = h('pre', {
        ...
        h('span', { key: 'text' }, screen === '' ? '(无输出)' : screen),
        // 隐藏样本：等宽字体下量 40 个 M，得到单元格宽度（绝对定位要用）
        h('span', {
