 ...
 * 这里只是在**已有文本节点**上取一个区间，量完即弃。
 *
 * @returns {number} 像素宽度；量不出来返回 NaN（调用方退回单元格估算，再不行就不画）
 */
function measurePrefixWidth(scope, offset) {
  try {
    ...
    const textNode = node === null ? null : node.firstChild
    ...
    const safe = Math.max(0, Math.min(textNode.textContent.length, offset))
...
/* 设置卡片（渲染在 DSH 设置页的插件区里） */
.dshsh-card{display:flex;flex-direction:column;gap:10px;color:var(--dsw-alias-label-primary);font-size:13px}
.dshsh-card-head{display:flex;align-items:center;gap:10px}
.dshsh-card-head strong{flex:1 1 auto;font-size:14px}
.dshsh-card-h{margin:6px 0 0;font-size:12px;font-weight:600;color:var(--dsw-alias-label-secondary)}
.dshsh-card-row{display:grid;grid-template-columns:minmax(150px,220px) minmax(120px,240px) 1fr;gap:10px;align-items:center}
.dshsh-card-label{display:flex;align-items:center;gap:6px}
.dshsh-card-label code{font-family:var(--ds-font-family-code);font-size:12px}
.dshsh-card-badge{padding:0 5px;border:0.5px solid var(--dsw-alias-state-warn-primary);border-radius:5px;color:var(--dsw-alias-state-warn-primary);font-size:11px;white-space:nowrap}
.dshsh-card-in{width:100%;padding:4px 8px;background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary);
  border:0.5px solid var(--dsw-alias-border-l2);border-radius:6px;font-family:var(--ds-font-family-code);font-size:12px}
.dshsh-card-in:disabled{opacity:.5}
.dshsh-card-desc{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:1.5}
.dshsh-card-note{color:var(--dsw-alias-label-secondary);font-size:12px}
.dshsh-card-err{color:var(--dsw-alias-state-error-primary);font-size:12px}
  ...
  factory: (require) => {
    ...
    function ShellPanel() {
      ...
      const caret = react.useMemo(() => {
        ...
        const measured = measurePrefixWidth(screenRef.current, placement.offset)
    ...
    /** 借来的原生卡片取值（来自同页其他插件卡片的实际实现，逐项对齐，见下方注释）。 */
    const CARD_CHROME = {
      border: '1px solid var(--dsw-alias-border-l2)',
      radius: '12px',
      header: { display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px' },
      title: { fontSize: '14px', fontWeight: 600 },
      sub: { color: 'var(--dsw-alias-label-tertiary)', fontSize: '12px', marginTop: '2px' },
      body: { padding: '0 16px 12px', display: 'flex', flexDirection: 'column', gap: '10px' },
      group: { color: 'var(--dsw-alias-label-secondary)', fontSize: '12px', fontWeight: 600, marginTop: '6px' },
      row: { display: 'grid', gridTemplateColumns: 'minmax(150px,220px) minmax(120px,240px) 1fr', gap: '10px', alignItems: 'center' },
      input: { width: '100%', padding: '8px 12px', borderRadius: '8px', border: '1px solid var(--dsw-alias-border-l2)',
        background: 'transparent', color: 'inherit', font: 'inherit' },
      badge: { border: '1px solid var(--dsw-alias-state-warn-primary)', color: 'var(--dsw-alias-state-warn-primary)',
        borderRadius: '6px', padding: '0 5px', fontSize: '11px', whiteSpace: 'nowrap' },
      desc: { color: 'var(--dsw-alias-label-tertiary)', fontSize: '12px', lineHeight: 1.5 },
      footer: { borderTop: '1px solid var(--dsw-alias-border-l2)', display: 'flex', alignItems: 'center',
        gap: '10px', padding: '12px 16px' },
      ghost: { border: '1px solid var(--dsw-alias-border-l2)', borderRadius: '8px', padding: '5px 14px',
        background: 'none', color: 'var(--dsw-alias-label-secondary)', font: 'inherit', cursor: 'pointer' },
      primary: { border: 0, borderRadius: '8px', padding: '6px 16px', font: 'inherit',
    ...
    function SettingsCard() {
      ...
      // 旋转 chevron：与同页原生卡片同款（同一个 16×16 路径与过渡）
      const chevron = h('svg', {
      ...
      for (const [title, keys] of CARD_GROUP) {
        ...
        rows.push(h('div', { key: 'h-' + title, style: CARD_CHROME.group }, title))
        for (const field of fields) {
          const value = valueOf(field)
          const control = field.type === 'boolean'
            ? h('input', {
                type: 'checkbox', checked: value === true, disabled: !state.writable,
                style: { justifySelf: 'start', width: '16px', height: '16px', accentColor: 'var(--dsw-alias-label-primary)' },
            ...
            : h('input', {
                ...
                style: { ...CARD_CHROME.input, fontFamily: 'var(--ds-font-family-code)', fontSize: '12px' },
                onChange: (event) => mark(field.key, event.target.value),
              })
          rows.push(h('div', { key: field.key, style: CARD_CHROME.row }, [
            h('div', { key: 'l', style: { display: 'flex', alignItems: 'center', gap: '6px', minWidth: 0 } }, [
              h('code', { key: 'k', style: { fontFamily: 'var(--ds-font-family-code)', fontSize: '12px' } }, field.key),
              field.restartRequired ? h('span', { key: 'r', style: CARD_CHROME.badge }, '需重启') : null,
            ]),
            control,
            h('div', { key: 'd', style: CARD_CHROME.desc }, field.description),
          ]))
        }
      }
      ...
      return h('div', {
        ...
        h('button', {
          key: 'header', type: 'button', 'aria-expanded': open,
          onClick: () => setOpen((previous) => !previous),
          style: { appearance: 'none', width: '100%', font: 'inherit', color: 'inherit', textAlign: 'left',
            cursor: 'pointer', background: 'none', border: 0, borderRadius: CARD_CHROME.radius, ...CARD_CHROME.header },
        }, [
          h('div', { key: 't', style: { flex: '1 1 auto', minWidth: 0 } }, [
            h('div', { key: 'title', style: CARD_CHROME.title }, '持久化 shell'),
            h('div', { key: 'sub', style: CARD_CHROME.sub }, summary),
          ]),
          chevron,
        ]),
        open ? h('div', { key: 'body', style: CARD_CHROME.body }, [
          state.error === '' ? null : h('div', { key: 'err', style: { color: 'var(--dsw-alias-state-error-primary)', fontSize: '12px' } }, '⚠ ' + state.error),
          state.writable ? null : h('div', { key: 'ro', style: CARD_CHROME.desc },
            '这个部署没有挂载设置服务：只能读，不能在这里改（请改用 cordis.patch.yml 的 config）。'),
          ...rows,
        ]) : null,
        open ? h('div', { key: 'footer', style: CARD_CHROME.footer }, [
          h('div', { key: 'note', style: { ...CARD_CHROME.desc, flex: '1 1 auto' } },
            state.error === '' ? state.note : ''),
          h('button', {
            key: 'discard', type: 'button', disabled: !dirty || saving,
            style: { ...CARD_CHROME.ghost, opacity: dirty && !saving ? 1 : .45, cursor: dirty && !saving ? 'pointer' : 'default' },
            onClick: () => setDraft({}),
          }, '放弃改动'),
          h('button', {
            key: 'save', type: 'button', disabled: !dirty || saving || !state.writable,
            style: { ...CARD_CHROME.primary, opacity: dirty && state.writable && !saving ? 1 : .4,
              cursor: dirty && state.writable && !saving ? 'pointer' : 'default' },
            onClick: () => { void save() },
          }, saving ? '保存中…' : '保存'),
        ]) : null,
      ])
