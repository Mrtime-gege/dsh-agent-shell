  ...
  factory: (require) => {
    ...
    function ShellPanel() {
      ...
      const caret = react.useMemo(() => {
        ...
        // 行高与字符宽度都必须来自**实际测量**；量不到就返回 null（caretStyle 内部判定），
        // 于是不画 —— 而不是用一个估出来的行高把光标画偏。
        const style = caretStyle(placement, lineHeightRef.current, cellWidthRef.current, SCREEN_PAD_X, SCREEN_PAD_Y / 2)
