...
.dshsh-info{position:absolute;top:40px;right:10px;z-index:41;width:min(400px,calc(100% - 20px));
  max-height:calc(100% - 96px);overflow:auto;padding:8px 10px 10px;border:1px solid var(--dsw-alias-border-primary,#2a2f3a);
  border-radius:10px;background:var(--dsw-alias-bg-primary,#16181d);box-shadow:0 14px 36px rgba(0,0,0,.55);
  font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;color:var(--dsw-alias-label-secondary,#94a3b8)}
.dshsh-info-group{margin:8px 0 2px;font-size:10px;letter-spacing:.04em;text-transform:uppercase;color:#64748b;
  border-bottom:1px solid rgba(148,163,184,.18);padding-bottom:3px}
.dshsh-info-group:first-child{margin-top:2px}
.dshsh-kv{display:flex;gap:8px;align-items:baseline;padding:2px 0;line-height:1.5}
.dshsh-kv>k{flex:0 0 96px;color:#64748b;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.dshsh-kv>v{flex:1 1 auto;min-width:0;color:var(--dsw-alias-label-primary,#e2e8f0);word-break:break-all}
.dshsh-kv>v.dim{color:#94a3b8}
.dshsh-kv>v.warn{color:#f87171}
.dshsh-kv>v.ok{color:#22c55e}
.dshsh-badge{display:inline-flex;align-items:center;height:16px;padding:0 6px;border-radius:999px;
  border:1px solid rgba(248,113,113,.45);color:#f87171;font-size:10px}
    ...
    function ShellPanel() {
      ...
      const info = infoOpen ? h('div', { key: 'info', className: 'dshsh-info' },
        buildInfoRows({
          ...
          h('div', { key: 'g-' + section.group, className: 'dshsh-info-group' }, section.group),
          ...section.rows.map((row) => h('div', { key: section.group + '-' + row.k, className: 'dshsh-kv' }, [
            h('k', { key: 'k', title: row.k }, row.k),
            h('v', {
              key: 'v',
              className: row.tone === undefined ? '' : row.tone,
              title: row.title === undefined || row.title === '' ? row.v : row.title,
            }, row.v),
          ])),
        ])) : null
