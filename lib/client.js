...
.dshsh-cmenu-tabs{display:flex;gap:4px;margin:0 2px 6px;padding:2px;border-radius:10px;background:var(--dsw-alias-button-ghost-active-fill)}
.dshsh-cmenu-tab{flex:1 1 auto;padding:4px 2px;border:0;border-radius:8px;background:transparent;
...
.dshsh-scope-card{display:flex;flex-direction:column;gap:2px;width:100%;padding:7px 10px;border-radius:10px;
  border:1px solid var(--dsw-alias-border-l2);background:transparent;color:var(--dsw-alias-label-primary);
  font:inherit;text-align:left;cursor:pointer;transition:border-color .12s ease,background .12s ease}
.dshsh-scope-card:hover{border-color:var(--dsw-alias-border-l4)}
.dshsh-scope-card.on{border-color:var(--dsw-alias-state-success-primary);background:var(--dsw-alias-button-ghost-active-fill)}
.dshsh-scope-title{font-size:12.5px;font-weight:600}
.dshsh-scope-hint{font-size:11px;color:var(--dsw-alias-label-tertiary);line-height:1.4}
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ShellPanel() {
      ...
      // 0.3.0「秘密与宏」页签：vault 人写（AI 只见键名）；宏人+AI 可写（全量入封链）
      ...
      const loadSecrets = async () => {
        setSecretsBusy(true)
        try {
          const [v, m] = await Promise.all([api('/vault-list'), api('/macro-list')])
          setSecretsData({
            vault: v.ok && Array.isArray(v.body.items) ? v.body.items : [],
            macros: m.ok && Array.isArray(m.body.items) ? m.body.items : [],
          })
        } catch (err) {
          setError('读取秘密与宏失败：' + String(err && err.message ? err.message : err))
        } finally {
      ...
      const secretsPost = async (path, payload, okMsg) => {
        ...
        try {
          ...
          else { setError(''); void loadSecrets() }
          return result.ok
        } catch (err) {
        ...
        h('button', {
          ...
          current === null
            ...
            : sessionCell(current, 'h-',
                // 通知 A：有新输出时亮 3 秒 —— 余光就知道「刚才那步出结果了」（触发器专属）
      ...
      const scopeCards = (prefix, value, setValue) => scopeKeys.map((key) => h('button', {
        key: prefix + key,
        type: 'button',
        className: 'dshsh-scope-card' + (value === key ? ' on' : ''),
        onClick: () => setValue(key),
        disabled: consentBusy,
        title: CONSENT_SCOPE_HINTS[key] ?? '',
      }, [
        h('span', { key: 't', className: 'dshsh-scope-title' }, CONSENT_SCOPE_LABELS[key] ?? key),
        h('span', { key: 'h', className: 'dshsh-scope-hint' }, CONSENT_SCOPE_HINTS[key] ?? ''),
      ]))
      // ── 0.3.0「秘密与宏」本体：vault（人写，AI 只引用键名）/ macros（人+AI，写入全入封链）──
      const secretsBody = h('div', { key: 'secrets', className: 'dshsh-cmenu-body' }, [
        h('div', { key: 'stabs', className: 'dshsh-cmenu-seg' }, [
          seg('sec-v', '秘密 vault', secretsTab === 'vault', () => setSecretsTab('vault')),
          seg('sec-m', '宏 macros', secretsTab === 'macros', () => setSecretsTab('macros')),
        ]),
        secretsData === null
          ? h('div', { key: 'ld', className: 'dshsh-cmenu-sub' }, '正在读取…')
          : secretsTab === 'vault' ? [
              h('div', { key: 'vn', className: 'dshsh-cmenu-sub' },
                '值只存宿主 vault.json（0600）；AI 只能引用 {{v:键名}}，值不上屏、不进审计。一次性 = 注入成功即焚。边界：output/ 留痕是原始字节流（见 SECURITY）。'),
              ...(secretsData.vault.length === 0
                ? [h('div', { key: 've', className: 'dshsh-cmenu-sub' }, '（还没有键）')]
                : secretsData.vault.map((item) => h('div', { key: 'vr-' + item.key, className: 'dshsh-cmenu-row' }, [
                    h('span', { key: 'k' }, '{{v:' + item.key + '}}'),
                    h('span', { key: 'm', className: 'dshsh-cmenu-sub' },
                      (item.oneShot ? '一次性' : '可重复') + ' · 用过 ' + item.uses + ' 次' + (item.note ? ' · ' + item.note : '')),
                    h('button', {
                      key: 'd', type: 'button', className: 'dshsh-cmenu-clear', disabled: secretsBusy,
              ...
              h('div', { key: 'va', className: 'dshsh-cmenu-actions' }, [
                h('button', {
                  ...
                  onClick: () => { void secretsPost('/vault-set', { key: vKey.trim(), value: vVal, oneShot: vOne, note: vNote.trim() }).then((ok) => { if (ok) { setVKey(''); setVVal(''); setVNote('') } }) },
                }, '保存秘密'),
              ]),
            ] : [
              ...
              ...(secretsData.macros.length === 0
                ...
                : secretsData.macros.map((item) => h('div', { key: 'mr-' + item.name, className: 'dshsh-cmenu-row' }, [
                    h('span', { key: 'k' }, '{{m:' + item.name + '}}'),
                    h('span', { key: 'm', className: 'dshsh-cmenu-sub' },
                      item.scope + ' · submit=' + (item.submit ? '是' : '否') + ' · ' + String(item.text).replace(/\n/g, '⏎').slice(0, 60)),
                    h('button', {
                      key: 'd', type: 'button', className: 'dshsh-cmenu-clear', disabled: secretsBusy,
              ...
              h('div', { key: 'mf1', className: 'dshsh-cmenu-row' }, [
                ...
                h('select', { key: 's', className: 'dshsh-cmenu-in', value: mScope, onChange: (e) => setMScope(e.target.value) }, [
                  ...
                  h('option', { key: 'c', value: 'conversation' }, 'conversation（仅面板侧）'),
                ]),
              ]),
      ...
      const consentMenu = consentMenuOpen ? h('div', { key: 'cmenu', className: 'dshsh-cmenu' }, [
        ...
        h('div', { key: 'tabs', className: 'dshsh-cmenu-tabs' }, [
          ...
          h('button', {
            ...
            onClick: () => { setConsentTab('secrets'); if (secretsData === null) void loadSecrets() },
            disabled: consentBusy,
            title: 'vault 秘密键（人写，AI 只引用 {{v:键名}}）与宏 {{m:名字}}（写入全入审计封链）',
          }, '秘密与宏'),
        ]),
        consentTab === 'secrets'
          ? secretsBody
