  ...
  factory: (require) => {
    ...
    function ShellPanel() {
      ...
      const stats = h('div', { key: 'stats', style: S.stats }, [
        h('span', { key: 'n' }, '名称 ' + (currentName || '-')),
        h('span', { key: 's' }, '尺寸 ' + (meta === null ? '-' : meta.cols + '×' + meta.rows)),
        h('span', { key: 'f' }, '前台 ' + (meta === null || !meta.foreground ? '-' : meta.foreground)),
        h('span', { key: 'b' }, '缓冲 ' + (meta === null ? '-' : meta.historySize + '/' + meta.historyLimit + ' 行')),
        h('span', { key: 'by' }, '已用 ' + (meta === null ? '-' : fmtBytes(meta.historyBytes))),
        h('span', { key: 'a' }, meta !== null && meta.attached ? '有人接入' : '无接入'),
        // 当前取景窗口：默认 200 行，点「更多历史」翻倍
        h('span', { key: 'w' }, '取景 ' + String(historyWindow) + ' 行'),
        h('button', {
          key: 'more',
          type: 'button',
          ...
          title: '向上多取一倍历史（上限 ' + String(MAX_WINDOW) + ' 行）',
        }, [h(Icon, { key: 'i', name: 'more', size: 12 }), h('span', { key: 't' }, '更多历史')]),
        // 翻上去之后才出现，一键回到底部
        pinned ? null : h('button', {
          ...
          title: '回到最新输出',
        }, [h(Icon, { key: 'i', name: 'latest', size: 12 }), h('span', { key: 't' }, '回到最新')]),
        server !== null ? h('span', { key: 'c' }, 'cwd ' + server.defaultCwd) : null,
        // 审批状态：必须**如实**标出。danger-full-access 下官方审批是 never —— 本插件的命令
        // 不会弹任何审批框，AI 可以执行任意命令。这条不是装饰，是风险告知。
        server !== null && server.approval !== undefined
          ? h('span', {
            key: 'ap',
            // 始终标红并写明「无审批」：这是**无条件为真**的事实 —— 无论会话策略是什么，
            // 本插件都不会向官方审批缝发起请求。（面板走 HTTP、拿不到会话，所以这里
            // 绝不显示「部署默认策略」来暗示存在审批，那会被误读。）
            style: { color: '#f87171', fontWeight: 600 },
            title: '⚠ 本插件未接入官方审批：模型执行的命令不会弹出任何「允许 / 拒绝」询问。'
              + '官方审批缝：' + String(server.approval.seam)
              + '；部署默认策略：' + String(server.approval.deploymentPolicy)
              + '（DSH_PERMISSION_MODE=' + String(server.approval.permissionMode) + '，单个会话可在运行期覆盖）。'
              + '想知道当前会话的确切策略请让模型调用 shell_diagnose。'
              + ' ' + String(server.approval.warning || ''),
          }, '无审批')
          : null,
        // 版本 + 构建号：确认浏览器跑的是哪一版（bundle URL 带 rev，只有重新加载页面才会换）
        h('span', { key: 'v', style: { opacity: 0.6 }, title: '插件版本 ' + PKG_VERSION + '，客户端构建 ' + BUILD + ' · 本插件由 AI 开发，未经人工安全审计。' },
          'v' + PKG_VERSION + ' · ' + BUILD),
      ].filter(Boolean))
      ...
      const inputRow = h('div', { key: 'input', style: S.inputRow }, [
        h('input', {
          ...
          placeholder: locked
            ? '🔒 输入已锁定 —— 点击锁图标解锁后可直接打字（防误触）'
            : '直接打字即进终端（支持输入法）：Tab 补全 · ↑↓ 历史 · Ctrl-R 搜索 · ←→ 移动',
      ...
      const children = [picker, head, body, stats, inputRow]
      if (error !== '') children.push(h('div', { key: 'err', style: S.err }, error))
