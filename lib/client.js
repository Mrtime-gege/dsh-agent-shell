...
function auditRowTitle(server) {
  ...
  if (chain !== null) {
    if (chain.ok) {
      lines.push(`哈希链校验通过：${String(chain.sealed)} 条封链${chain.legacy > 0 ? ' + ' + String(chain.legacy) + ' 条旧记录' : ''}${chain.startUnknown ? '（起点未知：更早的文件没读到）' : ''}；头 ${String(chain.head ?? '')}`)
    } else {
...
function chainRowValue(server) {
  ...
  if (chain.ok === true) {
    const extra = []
    if (Number.isFinite(chain.legacy) && chain.legacy > 0) extra.push(String(chain.legacy) + ' 条旧记录')
...
const ICON_PATHS = {
  ...
  sendall: 'M4 13l8 4v-3l8 3v-8l-8 3v-3z',
...
.dshsh-name{display:inline-flex;align-items:center;gap:5px;max-width:280px;padding:2px 7px 2px 10px;
...
.dshsh-compact .dshsh-name-owner,.dshsh-compact .dshsh-name-sub{display:none}
...
.dshsh-cmenu-h{margin:6px -4px 1px;padding:6px 10px 3px;position:sticky;top:0;z-index:1;
...
.dshsh-cmenu-row{display:flex;align-items:center;gap:8px;margin:0 6px 8px;padding:2px 2px}
...
.dshsh-cmenu-actions{display:flex;align-items:center;justify-content:flex-end;gap:6px;
...
.dshsh-item .dshsh-cell{display:flex;flex-direction:column;gap:2px;flex:1 1 auto;min-width:0;
  white-space:normal;overflow:visible}
.dshsh-item .dshsh-row-top{display:flex;align-items:center;gap:6px;min-width:0}
.dshsh-row-name{flex:0 1 auto;min-width:0;color:var(--dsw-alias-label-primary);
  font-weight:600;font-size:12px}
.dshsh-row-owner{flex:0 0 auto;max-width:55%;margin-left:auto;padding:0 5px;border-radius:999px;
  border:.5px solid var(--dsw-alias-border-l4);color:var(--dsw-alias-label-tertiary);
  font-size:10px;font-variant-numeric:tabular-nums}
.dshsh-item .dshsh-row-sub{display:flex;align-items:baseline;gap:8px;min-width:0;font-size:10.5px;
...
.dshsh-newout{color:var(--dsw-alias-state-success-primary);font-weight:600;font-variant-numeric:tabular-nums}
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ShellPanel() {
      ...
      const [newOut, setNewOut] = react.useState(0)
          ...
          try {
            ...
            if (result.ok) {
              ...
              if (next !== lastText) {
                lastText = next
                lastChange = Date.now()
                // 通知 A：屏幕行数增长 = 有新输出 → 亮 3 秒「+N 行」徽标
                const count = next === '' ? 0 : next.split('\n').length
                if (lastCount !== null && count > lastCount) {
                  setNewOut(count - lastCount)
                  if (notifyTimer.current !== null) clearTimeout(notifyTimer.current)
                  notifyTimer.current = setTimeout(() => { setJustIdle(false); setNewOut(0) }, 3000)
                }
                lastCount = count
              }
              setScreen(next)
      ...
      /**
       * 批处理一键发送：把输入框当前内容作为**命令**发到**全部** shell（各带 Enter）。
       * 复用同一个串行队列（enqueue）：逐个 /keys，顺序不乱、互不插队 —— 这正是
       * 「5 个 shell 同时跑同一段脚本」那类场景的入口。锁定时不放行（与 push 同语义）。
       */
      const broadcastAll = () => {
        const element = inputRef.current
        if (element === null || sessions.length === 0) return
        const text = element.value
        if (text === '') return
        element.value = ''
        setResidual(false)
        const names = sessions.map((s) => String(s.name ?? '')).filter((n) => n !== '')
        let failed = 0
        for (const name of names) {
          void enqueue(async () => {
            const result = await api('/keys', {
              method: 'POST',
              headers: { 'content-type': 'application/json' },
              body: JSON.stringify({ name, text, keys: ['Enter'] }),
            })
            if (result.body && result.body.refused === true) {
              failed += 1
              setError(`[${name}] ${result.body.message || 'refused'}`)
            } else if (!result.ok) {
              failed += 1
              setError(`[${name}] send failed`)
            }
          })
        }
        scheduleFastRefresh()
      ...
      // 创建它的会话（owner）：面板开的会带 (面板) 后缀，由宿主标注；未知就写「未知」
      ...
      const head = h('div', {
        ...
        h('button', {
          ...
          current === null
            ...
            : h('span', { key: 'cell', className: 'dshsh-name-cell' }, [
                h('span', { key: 'top', className: 'dshsh-name-top' }, [
                  h('span', { key: 'n', className: 'dshsh-name-label', title: rowDisplayName(current) }, rowDisplayName(current)),
                  h('span', {
                    key: 'ow',
                    className: 'dshsh-name-owner',
                    title: '创建这个 shell 的会话（面板里开的会标「面板」）',
                  }, '创建者：' + rowOwnerText(current)),
                ]),
                h('span', { key: 'sub', className: 'dshsh-name-sub' }, [
                  h('span', { key: 'id' }, current.name),
                  h('span', { key: 'm' },
                    String(current.cols) + '×' + String(current.rows)),
                  // 通知 A：有新输出时亮 3 秒 —— 余光就知道「刚才那步出结果了」
                  newOut > 0 ? h('span', { key: 'nw', className: 'dshsh-newout' }, '+' + String(newOut) + ' 行') : null,
                ]),
              ]),
        ...
        ...sessions.map((session) => {
          ...
          return h('div', {
            ...
            h('button', {
              ...
              h('span', { key: 'd', className: 'dshsh-dot', style: { background: dotColor(session) } }),
              h('span', { key: 'cell', className: 'dshsh-cell' }, [
                // 第一行：名字（显示名）+ 谁创建的（右对齐小徽标）
                h('span', { key: 'top', className: 'dshsh-row-top' }, [
                  h('span', { key: 'n', className: 'dshsh-row-name', title: rowDisplayName(session) }, rowDisplayName(session)),
                  h('span', {
                    key: 'ow',
                ...
                // 第二行：稳定 id + 尺寸/前台/字节
                h('span', { key: 'sub', className: 'dshsh-row-sub' }, [
                  h('span', { key: 'id', className: 'dshsh-row-id' }, session.name),
                  h('span', { key: 'm', className: 'dshsh-row-meta' },
      ...
      const inputRow = h('div', { key: 'input', style: S.inputRow }, [
        ...
        h('button', {
          key: 'broadcast',
          type: 'button',
          className: 'dshsh-btn' + (residual ? ' hot' : ''),
          // 批处理与 push 同源：都是「往终端送东西」，锁定时不放行；至少 2 个 shell 才有意义
          disabled: locked || currentName === '' || sessions.length < 2,
          onClick: broadcastAll,
          title: sessions.length < 2
            ? '批处理一键发送：需要至少 2 个 shell'
            : '批处理一键发送：把输入框内容作为命令发到全部 ' + String(sessions.length) + ' 个 shell（各带回车）',
        }, h(Icon, { name: 'sendall', size: 14 })),
      ])
      ...
      const consentMenu = consentMenuOpen ? h('div', { key: 'cmenu', className: 'dshsh-cmenu' }, [
        ...
        h('div', { key: 't3', className: 'dshsh-cmenu-h' }, '按对话授权（主动给指定会话）'),
        // 对话目录：搜索 → 选中 → 档位 × 有效期 → 授予。宿主没给目录（或读取失败）时，
        // 降级成「手工填 id」——授权仍可用，只是要自己粘贴会话 id（可从 shell_state/审计复制）。
        actors === null
          ...
          : actors.supported !== true
            ? h('div', { key: 'unsupported', className: 'dshsh-actors' }, [
                ...
                h('div', { key: 'mrow', className: 'dshsh-cmenu-actions' }, [
                  h('button', {
                    key: 'mg',
                    type: 'button',
                    className: 'dshsh-cmenu-go',
                    disabled: actorManual.trim() === '',
            ...
            : h('div', { key: 'act', className: 'dshsh-actors' }, [
                ...
                h('div', {
                  key: 'list',
                  className: 'dshsh-actor-list',
                  style: { maxHeight: '132px', overflow: 'auto' },
                  ...
                  .map((a) => h('button', {
                    ...
                    title: (a.title || a.id) + '（' + a.id + '）' + (a.granted ? '，已授权' : '，未授权') + (a.origin === 'subagent' ? '，子代理' : ''),
                  }, [
        ...
        actorPickBlock,
        h('div', { key: 't4', className: 'dshsh-cmenu-h' }, '已授权的会话（按最近使用排序）'),
