 ...
 * 把屏幕文本切成「光标前 / 光标后」两段。
 *
 * **为什么不用像素算**：早先的实现拿 `cursor_x`（单元格列）× 一个 ASCII 字符的实测宽度
 * 去算像素位置。那条路只在"CJK 字形宽度恰好是 ASCII 的 2 倍"时成立 —— 而中文通常由
 * **回退字体**渲染（advance 约 1em，而等宽字体的 ASCII 约 0.6em），文本实际比单元格网格窄，
 * 于是光标落在偏右的位置（实测用户报的"输入中文后光标偏后"就是这个）。
 *
 * 现在改成把插入点作为**内联元素插进文本流**：横向位置由浏览器排版给出，纵向由它所在的行给出 ——
 * 不再依赖任何字体度量，中文、emoji、回退字体、缩放全都不用管。
 */
function caretSplit(screen, placement) {
  const text = String(screen ?? '')
  if (placement === null || placement === undefined) return null
  const offset = Math.max(0, Math.min(text.length, placement.offset))
  return { before: text.slice(0, offset), after: text.slice(offset), offset }
}
...
/* 真实光标：锚点是文本流里的一个**零宽内联元素**（不占位、不影响排版），
   竖条绝对定位在锚点里 —— 横向位置由浏览器排版给出，因此中文/emoji/回退字体都不会偏。 */
.dshsh-caret-anchor{position:relative;display:inline;width:0}
.dshsh-caret{position:absolute;left:0;top:-.15em;width:2px;height:1.15em;background:currentColor;
  opacity:.8;border-radius:1px;pointer-events:none;animation:dshsh-blink 1.1s step-end infinite}
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ShellPanel() {
       ...
       *  注意：它只用于**滚动补偿**；光标不再依赖任何像素度量（见 caretSplit）。 */
      ...
      const caret = react.useMemo(() => {
        ...
        // 只做**文本切分**：位置交给浏览器排版（不再有任何像素/字体度量参与）
        return caretSplit(screen, placement)
      }, [screen, meta, locked])
      ...
      const body = h('pre', {
        ...
        ? (screen === '' ? '(无输出)' : screen)
        : [
            h('span', { key: 'before' }, caret.before),
            // 零宽锚点：不占位，插入点由它定位 —— 横向交给浏览器排版，纵向自然落在那一行
            h('span', { key: 'caret-anchor', className: 'dshsh-caret-anchor', 'aria-hidden': 'true' },
              h('span', { key: 'caret', className: 'dshsh-caret' })),
            h('span', { key: 'after' }, caret.after),
          ])
    ...
    function SettingsCard() {
      ...
      const save = react.useCallback(async () => {
        ...
        try {
          ...
          if (Object.keys(patch).length === 0) { setState((p) => ({ ...p, note: '没有改动' })); return }
          const response = await fetch(BASE + '/settings', {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
      ...
      if (state.status === 'loading') return h('div', { className: 'dshsh-card' }, '正在读取设置…')

      const byKey = new Map(state.fields.map((f) => [f.key, f]))
      const groups = CARD_GROUP.map(([title, keys]) => [title, keys.map((k) => byKey.get(k)).filter((f) => f !== undefined)])
      const rows = []
      for (const [title, fields] of groups) {
        if (fields.length === 0) continue
        rows.push(h('h4', { key: 'h-' + title, className: 'dshsh-card-h' }, title))
        for (const field of fields) {
          ...
          const control = field.type === 'boolean'
            ...
            : h('input', {
                ...
                className: 'dshsh-card-in',
                value: value === null || value === undefined ? '' : String(value),
                disabled: !state.writable,
                onChange: (event) => mark(field.key, event.target.value),
              })
          rows.push(h('div', { key: field.key, className: 'dshsh-card-row' }, [
            h('div', { key: 'l', className: 'dshsh-card-label' }, [
              h('code', { key: 'k' }, field.key),
              field.restartRequired ? h('span', { key: 'r', className: 'dshsh-card-badge' }, '需重启') : null,
            ]),
            control,
            h('div', { key: 'd', className: 'dshsh-card-desc' }, field.description),
          ]))
        }
      }

      return h('div', { className: 'dshsh-card' }, [
        h('div', { key: 'head', className: 'dshsh-card-head' }, [
          h('strong', { key: 't' }, '持久化 shell（dsh-agent-shell）'),
          h('button', {
            key: 'save', type: 'button', className: 'dshsh-btn accent', disabled: saving || !state.writable,
            onClick: () => { void save() },
          }, saving ? '保存中…' : '保存'),
          h('button', {
            key: 'reload', type: 'button', className: 'dshsh-btn', disabled: saving,
            onClick: () => { void load() },
          }, '重新读取'),
        ]),
        state.error === '' ? null : h('div', { key: 'err', className: 'dshsh-card-err' }, '⚠ ' + state.error),
        state.note === '' ? null : h('div', { key: 'note', className: 'dshsh-card-note' }, state.note),
        state.writable ? null : h('div', { key: 'ro', className: 'dshsh-card-note' },
          '这个部署没有挂载设置服务：只能读，不能在这里改（请用 cordis.patch.yml 的 config）。'),
        ...rows,
      ])
    }
