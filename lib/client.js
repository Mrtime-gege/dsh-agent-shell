    ...
    function ShellPanel() {
      ...
      const secretsBody = h('div', { key: 'secrets', className: 'dshsh-cmenu-body' }, [
        h('div', { key: 'stabs', className: 'dshsh-cmenu-seg' }, [
          seg('sec-v', '{{v: 秘密键}}', secretsTab === 'vault', () => { setSecretsTab('vault'); void loadSecrets() }),
          seg('sec-m', '{{m: 宏命令}}', secretsTab === 'macros', () => { setSecretsTab('macros'); void loadSecrets() }),
        ]),
        secretsData === null
      ...
      const consentMenu = consentMenuOpen ? h('div', { key: 'cmenu', className: 'dshsh-cmenu' }, [
        ...
        h('div', { key: 'tabs', className: 'dshsh-cmenu-tabs' }, [
          ...
          h('button', {
            ...
            onClick: () => { setConsentTab('secrets'); void loadSecrets() },   // 0.3.1：每次进入都重拉（外部烧键/CLI 改动即时可见；旧版只在首开拉一次，用完的键会滞留显示）
            disabled: consentBusy,
            title: '引用库：{{v:秘密键}}（人录入，AI 只见键名）+ {{m:宏}}（可复用命令片段，写入全入审计封链）',
          }, '引用库'),
        ]),
        consentTab === 'secrets'
          ? secretsBody
