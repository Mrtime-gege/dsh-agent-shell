...
## 0.2.2 — 批处理一键发送 + 互斥跨面板可见 + shell_wait 增量匹配 + 授权菜单按活跃排序

- **批处理一键发送**：面板输入框旁新增「发送到全部」按钮（至少 2 个 shell 才启用）——把输入框
  内容作为**命令**依次发到全部 shell（各带回车，走同一串行队列、顺序不乱），"5 个 shell 同时跑
  同一段脚本"这类场景一键完成。（工具层 `shell_send`/`shell_run` 本就接受逗号列表/`*`。）
