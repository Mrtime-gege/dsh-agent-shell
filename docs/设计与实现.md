...
三步走的目标：**能主动授权**（面板按钮 ✓ 已实现）→ **能限时**（15 分钟档已实现）→
**能把"时间 × 能力"两个维度都交给用户**（本节的模型）。
...
| 能力 | **完全控制**（不受限使用 shell）/ **可读**（只能看，不能输入）/ **完全禁止**（完全接触不到，但查授权状态不受影响） |

记录落在 `consent.json`，每个会话一条，外加一条 `*` 通配（面板的"所有对话"默认）：

```json
"<会话id>": { "title": "...", "scope": "full|read|deny", "expiresAt": 1789000000000,
              "at": 1789000000000, "by": "user", "source": "prompt|panel",
              "lastUsedAt": 1789000000000, "uses": 12 }
"*":        { "scope": "full", "expiresAt": null }        // null = 永久
```

`expiresAt` 为 `null` 表示永久；过期的条目与"没授权"等价（判定时顺手清理并审计记 `expired`）。

### 能力 → 工具的映射（本次定稿）

| 工具 | 完全控制 | 可读 | 完全禁止 |
|---|---|---|---|
| `shell_list` / `shell_read` / `shell_history` | ✓ | ✓ | ✗ |
| `shell_diagnose` / `shell_audit` | ✓ | ✓ | ✗ |
| `shell_consent`（查授权状态） | ✓ | ✓ | **✓**（查询不受影响，用户明确要求） |
| `shell_open` / `shell_send` / `shell_resize` / `shell_rename` | ✓ | ✗ | ✗ |
| **`shell_close`** | ✓ | **✗**（关闭会杀掉里面的进程，不算"只看看" —— 用户拍板） | ✗ |

**HTTP 路由不受这套能力门限制**：它们是面板（也就是你本人）的接口，且已有浏览器面闸门。
能力门管的是 **AI 能做什么**，不是"你能不能看自己的面板" —— 这一点写清楚，免得与"连诊断/审计也卡"
的直觉冲突：那条要求落实为"AI 用 `shell_diagnose`/`shell_audit` 时至少要 `read` 级"。

### 避免模型重复询问：三层

1. **拒绝/超时冷却**（关键的一层）：一个会话被拒绝或超时后，`consentRetryCooldownSeconds`
   （默认 600 秒）内**不再问、直接拒绝**，并告诉模型"用户已在 X 前拒绝/未回应，不要再问"。
   没有这一层时，模型会在每个工具调用前反复追问同一个问题。
2. **并发只问一次**：同一会话并行的工具调用复用同一个提问（已实现）。
3. **「完全禁止」是持久的"别再问"**：设成 `deny` 后该会话永久不再询问，直到用户改变。

### UI（面板头部授权按钮 → 浮层）

* 上半部「默认（所有对话）」：能力三档 + 时间五档 + 应用 —— 组合成 `完全控制 · 30 分钟` 这样的授权；
* 下半部「已授权会话」：**按最近使用排序**，显示**会话标题**（取自 `ctx.sessionQuery.readTitle()`，
  `SessionHeader` 本身没有名字，标题在日志的 `session/title` 事件里）+ 级别徽章 + 剩余时间，
  逐条可撤销；最近使用的那个标注「最近使用（很可能就是当前对话）」——
  这一条正是为了解决"会话多了列表很长、找不到当前会话"。
...
npm run test:client    # 270 项断言：keydown 判定与输入法组字状态机（零依赖，永远可跑）
npm run test:edge      # 248 项检查：参数边界/错误路径/护栏/HTTP 畸形输入（需 peer + tmux）
npm run smoke          # 打包产物冒烟：解包 → 假 ctx → 真实 tmux 跑通主流程（需 peer + tmux）
npm run release:check  # 发版不变量：版本/CHANGELOG/**只发必要文件**/README 链接/文档身份/peer/泄漏/客户端形态
npm run leak:history   # 全历史泄露扫描（含所有标签）；可带一个仓库路径，用来扫改写前的镜像做对照
