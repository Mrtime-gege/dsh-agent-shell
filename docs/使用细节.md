...
| 立即生效 | 需重启 `dsh web` |
|---|---|
| `watchdog` `shell` `cols` `rows` `maxSessions` `defaultCwd` `guardDangerousCommands` `allowedHosts` | `socket` `httpBase` `exposeHttp` `exposeTools` `defaultTerminal` `historyLimit` `extendedKeys` |
