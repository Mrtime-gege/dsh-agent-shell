...
| 立即生效 | 需重启 `dsh web` |
|---|---|
| `watchdog` `shell` `cols` `rows` `maxSessions` `defaultCwd` `guardDangerousCommands` | `socket` `httpBase` `exposeHttp` `exposeTools` `defaultTerminal` `historyLimit` `extendedKeys` |
...
开头、`shell` 不能为空。**越界会被当场拒绝并说明范围**；而组合配置（YAML）里写了越界值仍按
